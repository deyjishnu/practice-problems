...
void main() {
    ...
    s2[2] = s[2]; // This works because we can always change values of individual elements in an array
    puts(s); // This function and all other functions which takes a character array prints values of the array until null termination

    // CHARACTER POINTER
    char* s3;
    s3 = s; // This works because s3 is a pointer variable, and not a constant
