...
SELECT name, email, item, price
    FROM customers
    LEFT OUTER JOIN orders
    ON customers.id = orders.customer_id;

-- Now, create another query that will result in one row per each customer, with their name, email, and
-- total amount of money they've spent on orders. Sort the rows according to the total money spent, from the most spent to the least spent.
-- (Tip: You should always GROUP BY on the column that is most likely to be unique in a row.)
SELECT name, email, SUM(price) as total_money
    FROM customers
    LEFT OUTER JOIN orders
